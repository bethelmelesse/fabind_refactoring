...
args, args_new = get_arguments_from_command(command=command)
...
class Inference:
    ...
    def load_config(self):
        ...
        self.sdf_output_path_post_optim = "sdf_output_path_post_optim"  ##
        # train, valid, test: only native pocket.
        # train_after_warm_up, all_pocket_test include all other pockets(protein center and P2rank result)
        if args.redocking:
            args.compound_coords_init_mode = "redocking"
        elif args.redocking_no_rotate:
            args.redocking = True
            args.compound_coords_init_mode = "redocking_no_rotate"
        self.compound_coords_init_mode = args.compound_coords_init_mode

        self.index_csv = "inference_examples/example.csv"  ##
        self.pdb_file_dir = ""  ##
        self.preprocess_dir = "inference_examples"  ##
    ...
    def setup_directories(self):
        self.pre = os.path.join(self.result_folder, self.exp_name)
        os.makedirs(self.sdf_output_path_post_optim, exist_ok=True)
        os.makedirs(self.pre, exist_ok=True)
        log_path = os.path.join(self.pre, "test.log")
        self.logger = Logger(accelerator=self.accelerator, log_path=log_path)
        self.logger.log_message(f"{' '.join(sys.argv)}")
    ...
    def prep_dataset(self):
        ...
        self.logger.log_message(f"data point: {len(dataset)}")
