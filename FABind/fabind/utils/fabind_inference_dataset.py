...
class InferenceDataset(Dataset):
    def __init__(self, index_csv, pdb_file_dir, preprocess_dir):
        ...
        protein_path = os.path.join(preprocess_dir, "processed_protein.pt")
        self.protein_feature, self.protein_structure = torch.load(
            protein_path, weights_only=False
        )
        ...
        for i in tqdm(range(len(info))):
            ...
            try:
                ...
                mol_path = os.path.join(preprocess_dir, "mol", f"mol_{i}.pt")
                mol, molecule_info = torch.load(mol_path, weights_only=False)
            except:
    ...
    def get(self, idx):
        ...
        rdkit_coords, compound_node_features, input_atom_edge_list, LAS_edge_index = (
            input_dict["molecule_info"]
        )
        ...
        ].node_coords = torch.cat(  # [glb_c || compound || glb_p || protein]
            (
                torch.zeros(1, 3),
                coords_init
                - coords_init.mean(
        ...
        data[
            "complex_whole_protein"
        ].node_coords_LAS = torch.cat(  # [glb_c || compound || glb_p || protein]
            (
                torch.zeros(1, 3),
                rdkit_coords,
                torch.zeros(1, 3),
